...
def vectorizeAndDct(
    ...
    func = (lambda x: x.detach().clone()) if detach else (lambda x: x)
    if isinstance(src, list):
        flat = torch.cat([func(param).flatten() for param in src])
        dct_flat = dct.dct(flat)
        dct_flat = dct_flat[0: int(dct_flat.shape[-1] / 8)]
        return dct_flat
    elif isinstance(src, OrderedDict) or isinstance(src, dict):
        flat = torch.cat([func(param).flatten() for param in src.values()])
        dct_flat = dct.dct(flat)
        dct_flat = dct_flat[0: int(dct_flat.shape[-1] / 8)]

        return dct_flat
    elif isinstance(src, torch.nn.Module):
        flat =  torch.cat([func(param).flatten() for param in src.state_dict().values()])
        dct_flat = dct.dct(flat)
        dct_flat = dct_flat[0: int(dct_flat.shape[-1] / 8)]
