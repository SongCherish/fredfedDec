...
import numpy as np
from src.client.cfedrep import CFedRepClient
...
class CFedRepServer(FedAvgServer):
    algorithm_name: str = "CFedRep"
    ...
    def train_one_round(self):
        ...
        for indices in self.client_clusters:
            ...
            if (
                # mean_norm < self.args.cfedrep.eps_1
                # and max_norm > self.args.cfedrep.eps_2
                # and len(indices) > self.args.cfedrep.min_cluster_size
                # and
                    self.current_epoch == self.args.cfedrep.start_clustering_round
            ):
